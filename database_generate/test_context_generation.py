"""
Test script to verify context generation for executive summary
"""
import sqlite3
...
"""
    ...
    print(f"  - Number of lines: {len(context.split(chr(10)))}")
    
    print("\n✅ VALIDATION:")
    print(f"  - Has Demographics: {'✓' if 'Demographics' in context else '✗'}")
    print(f"  - Has Positive Themes: {'✓' if len(top_positive_themes) > 0 else '✗'}")
    print(f"  - Has Negative Themes: {'✓' if len(top_negative_themes) > 0 else '✗'}")
    print(f"  - Has Brand Data: {'✓' if len(brand_data) > 0 else '✗'}")
    print(f"  - Has Quotes: {'✓' if len(key_quotes) > 0 else '✗'}")
    print(f"  - No 'None' strings: {'✓' if 'None' not in context else '✗ (Found None!)'}")
    
    print("\n" + "=" * 80)
    print("CONTEXT GENERATION TEST COMPLETE")
    print("=" * 80)
